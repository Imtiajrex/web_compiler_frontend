...
struct Value
{
    ...
    Value &operator=(i64_t val)
    {
        _kind = Int_v;
        _val.integer = val;
        return *this;
    }
    Value &operator=(Fraction val)
    {
        _kind = Double_v;
        _val.floats = val;
        return *this;
    ...
    Value &operator=(StringLiteral val)
    {
        _kind = String_v;
        _val.strings = val;
        return *this;
    ...
    static void escape_string(FILE *des, char const *str, i32_t length)
    {
        ...
        for (i32_t i = 0; i < length; ++i)
        {
            ...
            {
                if (des == stdout)
                    std::cout << escape_character(str[i + 1]) <<std::flush;
                else
                    std::fprintf(des, "%c", escape_character(str[i + 1]));
                ++i;
                continue;
            }
            // print_output +=str[i];
    ...
    void print(FILE *des = stdout, bool escape = true)
    {
        ...
        case Int_v:
            // if (des == stdout)
                std::cout << static_cast<long long>(_val.integer) <<std::flush;
            // else
        ...
        case Char_v:
            ...
            {
                if (des == stdout)
                    std::cout << _val.charcter <<std::flush;
                // else
        ...
        case Bool_v:
            // if (des == stdout)
                std::cout << (_val.boolean ? "true" : "false") <<std::flush;
            // else
...
struct SymbolTable
{
    ...
    bool contains(char const *name, i32_t length, i32_t scope, i32_t &index, u8_t &count)
    {
        for (auto var = variables.rbegin(); var != variables.rend(); ++var)
        {
            ...
            if (var->scope == scope && var->length == length && std::strncmp(var->name, name, length) == 0)
            {
                index = var->index;
...
void print_error_line(int offset, char const *_text = text)
{
    ...
    for (i32_t i = 0; i < error_line.length; ++i)
    {
        ...
        if (error_line.text[i] == '\0')
            ...
            std::fprintf(stderr, "%c", error_line.text[i]);
...
void parse_primary_expression()
{
    ...
    case LeftParen:
    {
        ...
        if (is_cast)
        {
            ...
            {
            case Int_t:
                emit_single_byte(cast_to_int);
                break;
            case Double_t:
                emit_single_byte(cast_to_double);
                break;
            case Char_t:
                emit_single_byte(cast_to_char);
                break;
            case Bool_t:
                emit_single_byte(cast_to_bool);
                break;
            default:
                break;
            }
...
void parse_return_statement()
{
    ...
    {
        if (cur_local_index > 0)
        { // if return statement is in somewhere inner blocks
        ...
        { // if return statement is in the initial scope of the function
            for (auto i = 1; local_vars.size() >= i; ++i)
            {
                auto local = local_vars.at(local_vars.size() - i);
...
global_execution:
    ...
    runtime_start:
        ...
        case lt:
            ...
            relational_operation(<) else if (val1.is_double())
